...
public class FreedomPointsCounter {
    ...
    private FreedomLine getLineOfTheSameColorFrom(@NotNull FreedomLine tempLine, @NotNull Direction direction) {
        ...
        try {
            switch (direction) {
                case HORIZONTAL:
                    nextPosition = Position.valueOf(currentPosition.getRow(), currentPosition.getColumn() + 1);
                    break;
                case VERTICAL:
                    nextPosition = Position.valueOf(currentPosition.getRow() + 1, currentPosition.getColumn());
                    break;
                case DIAGONAL_LEFT:
                    nextPosition = Position.valueOf(currentPosition.getRow() + 1, currentPosition.getColumn() - 1);
                    break;
                default:
                    nextPosition = Position.valueOf(currentPosition.getRow() + 1, currentPosition.getColumn() + 1);
            }
    ...
    private Stone getThePreviousStone(@NotNull Position currentPosition, @NotNull Direction direction) throws InvalidPositionException {
        switch (direction) {
            case HORIZONTAL:
                return board.getStone(Position.valueOf(currentPosition.getRow(), currentPosition.getColumn() - 1));
            case VERTICAL:
                return board.getStone(Position.valueOf(currentPosition.getRow() - 1, currentPosition.getColumn()));
            case DIAGONAL_LEFT:
                return board.getStone(Position.valueOf(currentPosition.getRow() - 1, currentPosition.getColumn() + 1));
            default:
                return board.getStone(Position.valueOf(currentPosition.getRow() - 1, currentPosition.getColumn() - 1));
        }
    }
